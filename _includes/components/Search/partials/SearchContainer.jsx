...
export default class SearchContainer extends Component {
  ...
  sendGetRequest(newKeywords = this.state.keywords) {
    return new Promise((resolve, reject) => {
      fetch(`https://data.vulekamali.gov.za/api/3/action/package_search?q=${newKeywords}&start=0&rows=10&fq=+organization:national-treasury+vocab_financial_years:${this.props.selectedYear}`)
        .then((response) => {
          if (!response.ok) {
            response.text()
              .then((data) => {
